...
const actions = [
  { href: "/lecturer/manage-pdf", label: "Manage PDF", desc: "Organise course PDF materials.", accent: "#2563eb" },
  { href: "/lecturer/generate-quiz", label: "Generate Quiz", desc: "Draft quiz creation for the selected course.", accent: "#059669" },
  { href: "/lecturer/students", label: "Students", desc: "Review enrolled student lists.", accent: "#7c3aed" },
  { href: "/lecturer/notifications", label: "Notifications", desc: "Prepare course announcements.", accent: "#dc2626" },
];
  ...
  return (
    <div className="p-6 lg:p-8 space-y-8 max-w-[1400px]">
      <section
        ...
        style={{ background: "linear-gradient(135deg, #0e1f45 0%, #1a3262 50%, #1e3d72 100%)", minHeight: 160 }}
      >
        ...
        <div className="relative z-10 p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-black text-white mb-1">
              {selectedCourse ? `${selectedCourse.code} workspace` : "Welcome back to your lecturer dashboard"}
            </h1>
          ...
          <div className="rounded-2xl border px-5 py-4 min-w-[220px]" style={{ borderColor: "rgba(255,255,255,0.12)", background: "rgba(255,255,255,0.06)" }}>
            ...
            <div className="text-xs mt-0.5 truncate max-w-[240px]" style={{ color: "rgba(255,255,255,0.45)" }}>
              {selectedCourse?.name ?? "Use the header dropdown to choose a course."}
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
          {actions.map((action) => (
            <Link
              ...
              onClick={(e) => { if (!selectedCourse) e.preventDefault(); }}
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div className="w-11 h-11 rounded-xl flex items-center justify-center font-black" style={{ background: `${action.accent}12`, color: action.accent }}>
      ...
      <section className="bg-white rounded-2xl border p-6" style={{ borderColor: "#e2e8f0", boxShadow: "0 1px 3px rgba(0,0,0,0.05)" }}>
        ...
        <p className="text-sm mt-1 max-w-2xl" style={{ color: "#64748b" }}>
          The lecturer dashboard structure is ready. Course-specific modules are intentionally left as draft screens for the next content pass.
        </p>
      </section>
    </div>
  );
