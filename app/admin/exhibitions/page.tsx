...
function isAnyUpcoming(dates: string[]) {
...
function CreateModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  function setDate(i: number, val: string) {
    setDates((prev) => prev.map((d, j) => j === i ? val : d));
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setError("");
    if (!name.trim())           { setError("Exhibition name is required."); return; }
    if (!dates[0]?.trim())      { setError("At least one date is required."); return; }
    ...
    const res = await fetch("/api/admin/exhibitions", {
      ...
      body:    JSON.stringify({ name, dates: validDates, description, mainImage, subImages }),
    });
    setSaving(false);
    if (!res.ok) {
      const d = await res.json();
      setError(d.error ?? "Failed to create exhibition.");
      return;
    }
    onCreated();
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.55)" }}>
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-5 border-b" style={{ borderColor: "#f1f5f9" }}>
          <div>
            <h2 className="text-lg font-black" style={{ color: "#1a3262" }}>Create Exhibition</h2>
            <p className="text-xs mt-0.5" style={{ color: "#94a3b8" }}>Fill in the exhibition details below</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div>
            <label className="block text-xs font-bold mb-1.5" style={{ color: "#1a3262" }}>
              Exhibition Name <span style={{ color: "#dc2626" }}>*</span>
            </label>
            <input value={name} onChange={(e) => setName(e.target.value)}
              placeholder="e.g. Final Year Project Expo 2026"
          ...
          <div>
            <label className="block text-xs font-bold mb-1.5" style={{ color: "#1a3262" }}>
              Exhibition Date(s) <span style={{ color: "#dc2626" }}>*</span>
              <span className="ml-1 font-normal" style={{ color: "#94a3b8" }}>(add multiple if needed)</span>
            </label>
          ...
          <div>
            ...
            <textarea value={description} onChange={(e) => setDescription(e.target.value)}
              rows={3} placeholder="Describe the exhibition…"
...
/* ── Exhibition Card ──────────────────────────────────────────── */
function ExhibitionCard({ exh, onDelete }: { exh: ExhibitionItem; onDelete: (id: string) => void }) {
  const upcoming = isAnyUpcoming(exh.dates);
  const [confirmDelete, setConfirmDelete] = useState(false);
  const [deleting, setDeleting] = useState(false);

  async function handleDelete() {
    setDeleting(true);
    await fetch(`/api/admin/exhibitions/${exh._id}`, { method: "DELETE" });
    onDelete(exh._id);
  }
  ...
  return (
    <div className="bg-white rounded-2xl border overflow-hidden transition-shadow hover:shadow-md" style={{ borderColor: "#e2e8f0" }}>
      <div className="relative h-44 overflow-hidden">
        <img src={exh.mainImage} alt={exh.name} className="w-full h-full object-cover" />
        ...
        {exh.subImages.length > 0 && (
          <span className="absolute top-3 right-3 text-[10px] font-bold px-2 py-1 rounded-full"
            style={{ background: "rgba(0,0,0,0.55)", color: "white" }}>
            +{exh.subImages.length} photos
          </span>
        )}
        <div className="absolute bottom-3 left-3">
          <p className="text-white font-black text-base drop-shadow">{exh.name}</p>
        </div>
      </div>
      ...
      <div className="p-4">
        ...
        <div className="flex flex-wrap gap-2 mb-2">
          {exh.dates.map((d, i) => (
            <div key={i} className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold"
              style={{ background: "#f0f4ff", color: "#1a3262" }}>
        ...
        {exh.description && (
          <p className="text-xs leading-relaxed mb-3 line-clamp-2" style={{ color: "#64748b" }}>{exh.description}</p>
        )}

        {exh.subImages.length > 0 && (
          <div className="flex gap-1.5 mb-3 overflow-hidden">
            {exh.subImages.slice(0, 4).map((img, i) => (
...
export default function ExhibitionsPage() {
  ...
  const [exhs,      setExhs     ] = useState<ExhibitionItem[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    setExhs(await res.json());
    setLoading(false);
  }, [router]);

  useEffect(() => { fetchData(); }, [fetchData]);

  const upcoming = exhs.filter((e) => isAnyUpcoming(e.dates));
  const past     = exhs.filter((e) => !isAnyUpcoming(e.dates));

  return (
    <div className="p-6 lg:p-8 space-y-6 max-w-7xl">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-black" style={{ color: "#1a3262" }}>Exhibitions</h1>
          <p className="text-sm mt-0.5" style={{ color: "#64748b" }}>Create and manage university exhibitions and showcases.</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        {[
          { label: "Total Exhibitions", value: exhs.length,    color: "#1a3262" },
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 bg-white rounded-2xl border border-dashed" style={{ borderColor: "#e2e8f0" }}>
          ...
          <p className="font-bold mb-1" style={{ color: "#64748b" }}>No exhibitions yet</p>
          <p className="text-sm" style={{ color: "#94a3b8" }}>Create your first exhibition to get started.</p>
        </div>
      ) : (
        <>
          {upcoming.length > 0 && (
            <div>
              <h2 className="text-sm font-black uppercase tracking-wider mb-3" style={{ color: "#059669" }}>Upcoming Exhibitions</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {upcoming.map((e) => (
                  <ExhibitionCard key={e._id} exh={e} onDelete={(id) => setExhs((prev) => prev.filter((x) => x._id !== id))} />
                ))}
              </div>
            </div>
          )}
          {past.length > 0 && (
            <div>
              <h2 className="text-sm font-black uppercase tracking-wider mb-3 mt-4" style={{ color: "#64748b" }}>Past Exhibitions</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {past.map((e) => (
                  <ExhibitionCard key={e._id} exh={e} onDelete={(id) => setExhs((prev) => prev.filter((x) => x._id !== id))} />
                ))}
